...
FOR %%x IN ("\GitKraken\MainSetUp\not_hacking(yet)\xampp\php\pear") DO (if %%x=="%PMTPMTPMT%" GOTO :NOTINSTALLED)

REM Check PEAR global ENV, set them if they do not exist
IF "%PHP_PEAR_INSTALL_DIR%"=="" SET "PHP_PEAR_INSTALL_DIR=\GitKraken\MainSetUp\not_hacking(yet)\xampp\php\pear"
IF "%PHP_PEAR_BIN_DIR%"=="" SET "PHP_PEAR_BIN_DIR=\GitKraken\MainSetUp\not_hacking(yet)\xampp\php"
IF "%PHP_PEAR_PHP_BIN%"=="" SET "PHP_PEAR_PHP_BIN=\GitKraken\MainSetUp\not_hacking(yet)\xampp\php\php.exe"
GOTO :INSTALLED
